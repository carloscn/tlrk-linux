...
#define IMX327_LINK_FREQ_445M		445500000
#define IMX327_2LANES			2
#define IMX327_4LANES			4
#define IMX327_BITS_PER_SAMPLE		10

/* pixel rate = link frequency * 2 * lanes / BITS_PER_SAMPLE */
#define IMX327_PIXEL_RATE_NORMAL (2200 * 1125 * 60) // vts * hts * fps
 ...
 * Xclk 74.25Mhz
 * max_framerate 60fps
 * mipi_datarate per lane 445.5Mbps 2 lane
 */
static const struct regval imx327_linear_1920x1080_mipi_regs_2lane_60fps[] = {
	{0x3003, 0x01},
	{REG_DELAY, 0x10},
	{0x3000, 0x01},
	{0x3001, 0x00},
	{0x3002, 0x01},
	{0x3005, 0x00},	// ADBIT 0: 10 bit, 1: 12 bit
	{0x3007, 0x00},	// [1:0] [HREVERSE, VREVERSE] [0h: Normal, 1h: Inverted]
	{0x3009, 0x01},	// [1:0] 2h: 2lane 30fps, 1h: 2lane 60fps; [4] 0h:LCG mode, 1h: HCG mode
	{0x300A, 0x3c},
	{0x3010, 0x21},
	{0x3011, 0x0a},
	{0x3018, 0x46},	// VMAX = 1125
	{0x3019, 0x05},	// VMAX = 1125
	{0x301C, 0x98},	// 30h: 30fps, A0h: 25fps, 98h: 60fps, 50h: 50fps
	{0x301D, 0x08},	// 11h: 30fps, 14h: 25fps, 08h: 60fps, 0Ah: 50fps
	{0x3046, 0x00},	// ODBIT 0: 10 bit, 1: 12 bit
	{0x304B, 0x0A},
	{0x305C, 0x0c},	// INCKSEL1 0Ch: 74.25MHz, 18h: 37.125 MHz
	{0x305D, 0x03},	// INCKSEL2 03h: 74.25MHz, 03h: 37.125 MHz
	{0x305E, 0x10},	// INCKSEL3 10h: 74.25MHz, 20h: 37.125 MHz
	{0x305F, 0x01},	// INCKSEL4 01h: 74.25MHz, 01h: 37.125 MHz
	{0x309E, 0x4A},
	{0x309F, 0x4A},
	{0x311c, 0x0e},
	{0x3128, 0x04},
	{0x3129, 0x1d},	// ADBIT1 10 bit: 1Dh, 12 bit: 00h
	{0x313B, 0x41},
	{0x315E, 0x1b},	// INCKSEL5 1Bh: 74.25MHz, 1Ah: 37.125 MHz
	{0x3164, 0x1b},	// INCKSEL6 1Bh: 74.25MHz, 1Ah: 37.125 MHz
	{0x317C, 0x12},	// ADBIT2 12h: 10 bit, 00h: 12 bit
	{0x31EC, 0x37},	// ADBIT3 37h: 10 bit, 0Eh: 12 bit
	{0x3405, 0x00}, // 1h: 2lane, Date rate: 445.5Mbps, 30/25fps; 0h: 2lane, Date rate: 891Mbps, 60/50fps
	{0x3407, 0x01}, // 1h: 2lane, 3h: 4lane
	{0x3414, 0x0A},
	{0x3418, 0x49},
	{0x3419, 0x04},
	{0x3441, 0x0a},	// 0A0Ah: RAW10, 0C0Ch: RAW12
	{0x3442, 0x0a},
	{0x3443, 0x01}, // 1h: 2lane, 3h: 4lane
	{0x3444, 0x40},	// EXTCK_FREQ 20h: 37.125 MHz, 40h: 74.25 MHz
	{0x3445, 0x4a},	// EXTCK_FREQ 25h: 37.125 MHz, 4Ah: 74.25 MHz
	{0x3446, 0x77},	// 57h: 2lane, Date rate: 445.5Mbps, 30/25fps; 77h: 2lane, Date rate: 891Mbps, 60/50fps
	{0x3447, 0x00},	// Same as 0x3445
	{0x3448, 0x67},	// 37h: 2lane, Date rate: 445.5Mbps, 30/25fps; 67h: 2lane, Date rate: 891Mbps, 60/50fps
	{0x3449, 0x00},	// Same as 0x3448
	{0x344A, 0x47},	// 01Fh: 2lane, Date rate: 445.5Mbps, 30/25fps; 047h: 2lane, Date rate: 891Mbps, 60/50fps
	{0x344B, 0x00},	// Same as 0x344A
	{0x344C, 0x37},	// 01Fh: 2lane, Date rate: 445.5Mbps, 30/25fps; 037h: 2lane, Date rate: 891Mbps, 60/50fps
	{0x344D, 0x00},	// Same as 0x344C
	{0x344E, 0x3F},	// 01Fh: 2lane, Date rate: 445.5Mbps, 30/25fps; 03Fh: 2lane, Date rate: 891Mbps, 60/50fps
	{0x344F, 0x00},	// Same as 0x344E
	{0x3450, 0xFF},	// 077h: 2lane, Date rate: 445.5Mbps, 30/25fps; 0FFh: 2lane, Date rate: 891Mbps, 60/50fps
	{0x3451, 0x00},	// Same as 0x3450
	{0x3452, 0x3F},	// 01Fh: 2lane, Date rate: 445.5Mbps, 30/25fps; 03Fh: 2lane, Date rate: 891Mbps, 60/50fps
	{0x3453, 0x00},	// Same as 0x3452
	{0x3454, 0x37},	// 017h: 2lane, Date rate: 445.5Mbps, 30/25fps; 037h: 2lane, Date rate: 891Mbps, 60/50fps
	{0x3455, 0x00},	// Same as 0x3454
	{0x3472, 0x9c},
	{0x3473, 0x07},
	{0x3480, 0x49},	// INCKSEL6 49h: 74.25MHz, 92h: 37.125 MHz
	{0x3002, 0x00},
	{REG_NULL, 0x00},
};
...
static const struct imx327_mode mipi_supported_modes[] = {
	{	/* 1080p 60fps */
		...
		.exp_def = 0x03fe,
		.hts_def = 0x0898,	// 3168: 25fps, 2640: 30fps, 3168: 50fps, 2640: 60fps
		.vts_def = 0x0465,	// 1125
		.reg_list = imx327_linear_1920x1080_mipi_regs_2lane_60fps,
		.hdr_mode = NO_HDR,
	},
	{
		.bus_fmt = MEDIA_BUS_FMT_SRGGB10_1X10,
