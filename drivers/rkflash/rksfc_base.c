...
static int rksfc_probe(struct platform_device *pdev)
{
	...
	u32 status;
#endif
	spidev_mode = of_property_read_bool(pdev->dev.of_node, "spidev_mode");
	...
	/*
	 * If spidev_mode is enabled, do not tune delay lines,
	 * because delay lines tuning base on reading nor/nand flash id.
	 */
	if (sfc_get_version() >= SFC_VER_4 && \
		g_sfc_info.clk_rate > RKSFC_DLL_THRESHOLD_RATE && !spidev_mode)
		rksfc_delay_lines_tuning();
	else if (sfc_get_version() >= SFC_VER_4)
		sfc_set_delay_lines(0);

#ifdef CONFIG_RK_SFC_NOR
	/* If spidev_mode is enabled, do not init sfc for nor flash */
	if (!spidev_mode)
		dev_result = rkflash_dev_init(g_sfc_info.reg_base, FLASH_TYPE_SFC_NOR, &sfc_nor_ops);
#endif
#ifdef CONFIG_RK_SFC_NAND
	/* If spidev_mode is enabled, do not init sfc for nand flash */
	if (!spidev_mode && dev_result)
		dev_result = rkflash_dev_init(g_sfc_info.reg_base, FLASH_TYPE_SFC_NAND, &sfc_nand_ops);
#endif

	/* Register rksfc spi controller pdev */
	if (spidev_mode) {
		struct rksfc_data *data;
		struct platform_device *spi_controller_pdev;

		data = kzalloc(sizeof(struct rksfc_data), GFP_KERNEL);
		if (!data) {
			dev_err(&pdev->dev, "Failed to allocate rksfc data\n");
			return -ENOMEM;
		}

		data->spi_bus_num = of_alias_get_id(pdev->dev.of_node, "spi");
		if (data->spi_bus_num < 0) {
			dev_err(&pdev->dev, "SPI failed to get alias id\n");
			kfree(data);
			return -EINVAL;
		}
		data->sfc_clk = g_sfc_info.clk;

		spi_controller_pdev = platform_device_alloc("rksfc_spi_controller",
			PLATFORM_DEVID_NONE);
		if (!spi_controller_pdev) {
			dev_err(&pdev->dev, "Failed to allocate rksfc spi controller pdev\n");
			kfree(data);
			return -ENOMEM;
		}

		dev_result = platform_device_add_data(spi_controller_pdev,
			(void *)data, sizeof(*data));
		if (dev_result != 0) {
			dev_err(&pdev->dev, "Failed to add data for rksfc spi controller pdev: %d\n",
				dev_result);
			platform_device_put(spi_controller_pdev);
			kfree(data);
		}

		dev_result = platform_device_add(spi_controller_pdev);
		if (dev_result != 0) {
			dev_err(&pdev->dev, "Failed to register rksfc spi controller pdev: %d\n",
				dev_result);
			platform_device_put(spi_controller_pdev);
			kfree(data);
		}
