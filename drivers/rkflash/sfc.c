...
EXPORT_SYMBOL_GPL(sfc_get_version);
...
EXPORT_SYMBOL_GPL(sfc_set_delay_lines);
...
int sfc_request_by_fifo(struct rk_sfc_op *op, u32 addr, const void *w_data, void *r_data, u32 size)
{
	int ret = SFC_OK;
	union SFCCMD_DATA cmd;
	int reg;
...
EXPORT_SYMBOL_GPL(sfc_request_by_fifo);

int sfc_request_by_dma(struct rk_sfc_op *op, u32 addr, const void *w_data, void *r_data, u32 size)
{
	int ret = SFC_OK;
	union SFCCMD_DATA cmd;
	int reg;
	...
	dma_addr = rksfc_dma_map_single(direction ? \
					(unsigned long)w_data : (unsigned long)r_data,
					size,
					direction);
	rksfc_irq_flag_init();
	writel(0xFFFFFFFF, g_sfc_reg + SFC_ICLR);
...
EXPORT_SYMBOL_GPL(sfc_request_by_dma);
