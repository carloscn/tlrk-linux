...
static int dw8250_probe(struct platform_device *pdev)
{
	...
	/* check for rs485 enable gpio */
	uart.rs485_en_gpio = of_get_named_gpio_flags(np, "rs485-en-gpios", 0, &flags);
	if (uart.rs485_en_gpio == -EPROBE_DEFER)
		return -EPROBE_DEFER;

	if (gpio_is_valid(uart.rs485_en_gpio)){
		err = devm_gpio_request(dev, uart.rs485_en_gpio, "serial_rs485_en_gpio");
		if (err < 0)
			return err;

		dev_info(dev, "have rs485-en-gpio, request rs485-en-gpio successful \n");
		uart.rs485_en_gpio_valid = true;

		err = gpio_direction_output(uart.rs485_en_gpio, flags & SER_RS485_RTS_AFTER_SEND);
		if (err < 0)
			return err;
	}
