...
static void serial8250_start_tx(struct uart_port *port)
{
	...
	if (up->rs485_en_gpio_valid) {
		if (!(gpio_get_value(up->rs485_en_gpio))) {
			gpio_set_value(up->rs485_en_gpio, 1);
		}
	}
...
void serial8250_tx_chars(struct uart_8250_port *up)
{
	...
	u8 *usr_reg = port->private_data;
	int count, timeout, usr = 0;
	...
	if (uart_circ_empty(xmit)) {
		if (up->rs485_en_gpio_valid) {
			do {
				usr = port->serial_in(port, *usr_reg);
				timeout++;
				if (timeout > 500000)
					break;
			} while (usr & 1);
			...
			if (gpio_get_value(up->rs485_en_gpio)) {
				gpio_set_value(up->rs485_en_gpio, 0);
			}
		}
		__stop_tx(up);
		return;
